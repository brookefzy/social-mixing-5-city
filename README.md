...
The code in this repo is organized as notebook-based figure reproduction. The required study data are available directly under the repo-local `data/` directory.

## Repository layout

- `script/`: Jupyter notebooks used to reproduce the figures currently ported into this repo
- `data/`: source parquet and CSV files used by the notebooks
...
The study data are checked in under `data/`. Your top-level folder should include:

```text
data/
  c_individual_all.csv
  c_individual_all.parquet
  c_individual_all_osm_home_15_place_15.csv
